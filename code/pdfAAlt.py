...
class pdfAA:
    ...
    def pullCityYearMonthValues(self, pageText):
        """input: all the data
        ...
        for month in self.months:
            if month.lower() in pageText.lower():
    ...
    def pullRowNameAndValues(self, rowText,  rowIndices):
        """
        ...
        return None
    def match(self, rowValues , rowIndex, found_year, found_months, found_cities):
        ...
        one = [found_cities[0], found_year,
               found_months[0], rowIndex, rowValues[4]]
        two = [found_cities[0], found_year,
               found_months[1], rowIndex, rowValues[3]]
        three = [found_cities[1], found_year,
                 found_months[0], rowIndex, rowValues[1]]
        four = [found_cities[1], found_year,
                found_months[1], rowIndex, rowValues[0]]
        if one:
            return (one, two, three, four)
    def processPages(self, pdf_path):
        ...
        for pageText in text:
            if self.checkPageType(pageText):
                ...
                rowValues = self.pullRowNameAndValues(self.rowIndices, pageText)
                if rowValues:  # If rowValues is not None
                    for rowIndex in self.rowIndices: 
                        full_rows = self.match(
                            rowValues, rowIndex, found_year, found_months, found_cities)
